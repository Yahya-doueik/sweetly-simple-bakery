...
function CartProvider({ children }) {
  ...
  const addItem = reactExports.useCallback((cake, quantity = 1) => {
    setItems((prev) => {
      ...
      if (existing)
        return prev.map(
          (i) => i.cake.id === cake.id ? { ...i, quantity: i.quantity + quantity } : i
        );
  ...
  const setQuantity = reactExports.useCallback((id, quantity) => {
    setItems(
      (prev) => quantity <= 0 ? prev.filter((i) => i.cake.id !== id) : prev.map((i) => i.cake.id === id ? { ...i, quantity } : i)
    );
  }, []);
...
const $$splitComponentImporter$2 = () => import("./checkout-DtzRGbCT.mjs");
...
const $$splitComponentImporter = () => import("./index-BB86doxn.mjs");
